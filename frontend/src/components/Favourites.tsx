...
export default function Favourites() {
    ...
    const fetchFavourites = async () => {
        try {
            ...
            const mappedData: Repository[] = await Promise.all(
                res.data.data.map(async (repo: FavoriteResponse) => {
                    let owner = { login: "unknown", avatar_url: "" };

                    if (!repo.owner && repo.repoId) {
                        const githubRes = await axios.get(`https://api.github.com/repositories/${repo.repoId}`);
